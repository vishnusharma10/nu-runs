...
const ChallengeCard = ({ challenge, distance, challengeType, id }) => {
  const getEnrolled = async (userId, challengeId) => {
    ...
    const result = await axios.post(
      "http://localhost:8000/api/profile/challenge-enroll/" +
        userId +
        "/" +
        challengeId
    );
  ...
  const fetchChallengeImage = async (id) => {
    const result = await axios.get(
      "http://localhost:8000/api/challenges/challenge-img/" + id,
    ...
    const newImgUrl = `data:${result.headers[
      "content-type"
        ...
        <div>
          <div class="speaker">
            <div class="speaker-img" data-triangle=".speaker-overlay">
              <a href="pricing.php">
                <img src={imgUrl} alt="" width="330" height="354"></img>
              </a>
            </div>
            <h5>
              <p>{distance} KM Run</p>
